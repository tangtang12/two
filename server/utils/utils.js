...
function ADD_STORE(req, res, { shopId, num, size, color }) {
  ...
  let { price } = req.courseDATA.find(item => item.id === shopId);
  let storeInfo = {
    id:
      req.storeDATA.length === 0
        ? 1
        : parseFloat(req.storeDATA[req.storeDATA.length - 1].id) + 1,
    shopId,
    num,
    size,
    color,
